...
export default function HomePage() {
	...
	return (
		<div>
			{/* Hero */}
			<section className="bg-gradient-to-br from-secondary to-background py-16 md:py-24 border-b border-border">
				<div className="container mx-auto px-4 text-center">
					<h1 className="text-3xl md:text-5xl font-bold mb-4">
						Znajdź to, czego szukasz
					</h1>
					<p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
						Tysiące ogłoszeń w jednym miejscu. Kupuj i sprzedawaj bezpiecznie.
					</p>
					...
					<div className="flex gap-3 overflow-x-auto pb-2">
						{categories.map((cat) => (
							<Link
								...
								className="flex-shrink-0 rounded-xl border border-border bg-card px-5 py-3 text-sm font-medium hover:bg-secondary transition-colors"
							>
								{cat.name}
							</Link>
						))}
			...
			<section className="container mx-auto px-4 py-12">
				<div className="flex items-center justify-between mb-6">
					...
					<Button variant="ghost" asChild>
						<Link to="/items" className="flex items-center gap-1">
							...
							<Link key={item.id} to={'/items/' + item.id}>
								<Card className="overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:shadow-primary/5">
									<div className="relative aspect-[4/3] bg-muted">
										...
										>
											{item.condition === ItemCondition.New ? 'Nowy' : 'Używany'}
										</Badge>
									</div>
